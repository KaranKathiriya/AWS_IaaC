...
cp $currDir/templates/logserver_cloudformation.tmpl.yml $outputDir/logserver_cloudformation.yml


#
# Replace YML template variables with values

sed -i "s!{KEY_PAIR_NAME}!$KEY_PAIR_NAME!g" $outputDir/logserver_cloudformation.yml
sed -i "s!{AWS_AVAILABILITY_ZONE}!$AWS_AVAILABILITY_ZONE!g" $outputDir/logserver_cloudformation.yml